...
in
{
  ...
  config = mkIf cfg.enable {
    assertions = [
      (
        let
          defaults = lib.catAttrs "name" (lib.filter (a: a.isDefault) (lib.attrValues cfg.profiles));
        in
        {
          assertion = cfg.profiles == { } || builtins.length defaults == 1;
          message =
            "Must have exactly one default Zotero profile but found "
            + toString (builtins.length defaults)
            + lib.optionalString (builtins.length defaults > 1) (", namely " + concatStringsSep ", " defaults);
        }
      )

      (
        let
          duplicates = lib.filterAttrs (_: v: builtins.length v != 1) (
            lib.zipAttrs (mapAttrsToList (n: v: { "${toString v.id}" = n; }) cfg.profiles)
          );
